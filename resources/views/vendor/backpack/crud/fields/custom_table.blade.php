            ...
            <tbody class="table-striped items sortableOptions">
                ...
                <tr class="array-row clonable" style="display: none">
                    @foreach( $field['columns'] as $column => $label)
                        @if($column == 'image')
                            <td>
                                <input class="form-control form-control-sm" type="file" name="images[]" data-cell-name="item.{{ $column }}">
                            </td>
                        @else
                        <td>
                            <input class="form-control form-control-sm" type="text" name="titles[]" data-cell-name="item.{{ $column }}">
                        </td>
                        @endif
                    @endforeach
...
@if ($crud->fieldTypeNotLoaded($field))
    ...
    @push('crud_fields_scripts')
        ...
        <script>
            $(document).ready(function() {
                ...
                // let inp_browse = $("input").find("[data-cell-name='item.image']");
                // inp_browse.attr('type', 'file');
            });
    ...
    @push('crud_fields_styles')
        ...
        <style>
            .form-control-sm {height: calc(2em + 0.5rem + 2px);}
        </style>
    @endpush
@endif

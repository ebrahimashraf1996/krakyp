...
@section('content')
<?php
function tree_element($entry, $key, $all_entries, $crud, $id)
{
...
//dd($id);
        ...
        $attr = \App\Models\Attribute::where('id', object_get($entry, $crud->get('reorder.label')))->first();

        echo '<li id="list_'.$attr->id.'">';
        echo '<div><span class="disclose"><span></span></span>'. '<span class="px-3" style="margin-right:20px;margin-left:20px;color: green">'. $attr->type_of .'</span>'. $attr->title . '<span class="px-3 text-danger">'. $attr->type .'</span>'.'</div>';

        // see if this element has any children
//dd($clids  = \Illuminate\Support\Facades\DB::table('attr_cat')->where('cat_id', $id)->where('parent_id', 84)->orderBy('lft', 'ASC')->get());

        $children = [];
        //dd($all_entries);
        foreach ($all_entries as $key => $subentry) {
            if ($subentry->parent_id == $attr->id) {
                $children[] = $subentry;
            }
        }
        ...
        if (count($children)) {
            ...
            foreach ($children as $key => $child) {
                $children[$key] = tree_element($child, $key, $all_entries, $crud, $id);
            }
            echo '</ol>';
        }
        echo '</li>';
    ...
    <div class="{{ $crud->getReorderContentClass() }}">
        <div class="card p-4">
            ...
            <?php
                ...
                // dd($id);
                $all_entries = \Illuminate\Support\Facades\DB::table('attr_cat')->where('cat_id', $id)->orderBy('lft', 'ASC')->get();
//                dd($all_entries);
//                dd($all_entries['8']->attributes[0]);
                $root_entries = $all_entries->filter(function ($item) {
                    return $item->parent_id == 0;
                });
                foreach ($root_entries as $key => $entry) {
                    $root_entries[$key] = tree_element($entry, $key, $all_entries, $crud, $id);
                }
//                dd($key . $all_entries . $crud);
            ?>
            </ol>
